...
class Conv2dClassifier(nn.Module):
    def __init__(self, c_in=1, num_classes=7):
        ...
        # 48x48x1
        self.conv1 = ConvBlock(c_in, 128, kernel_size=3, stride=1, padding=1)
        self.max_pool = MaxPool2d(kernel_size=2, stride=2)
        # 48x48x32
        self.conv2 = ConvBlock(128, 256, kernel_size=3, stride=1, padding=1)
        self.max_pool = MaxPool2d(kernel_size=2, stride=2)
        # 24x24x64
        self.conv3 = ConvBlock(256, 512, kernel_size=3, stride=1, padding=1)
        self.max_pool = MaxPool2d(kernel_size=2, stride=2)
        # 12x12x128
        self.conv4 = ConvBlock(512, 1024, kernel_size=3, stride=1, padding=1)
        # 6x6x256

        self.classifier = nn.Linear(1024 * 6 * 6, num_classes)
